        ...
        <div class="card w-full md:w-1/4">
            <div class="card-image h-80 bg-black">
                ...
                <a href="/projects/{{$project->id}}"
                    class="btn-floating halfway-fab waves-effect waves-light red btn-large"><i
                        class="material-icons">remove_red_eye</i></a>
            </div>
            <div class="card-content">
                <p class="pb-4">{{ $project->description }}</p>
                @if ($project->timestamp->first())
                <span class="mb-2"><b>Time spent:</b>
                    {{$project->timestamp->first()->getTotalTimeSpent($project->id)}}</span>
                @else
                <span class="mb-2"><b>Time spent:</b> Not started</span>
                @endif
                <br />
                @if ($project->timestamp->first())
                <span class="mb-2"><b>Billable:</b>
                    {{$project->timestamp->first()->getTotalCharges($project->id)}}</span>
                @else
